...
更新于 2026-09-13：在之前的有效性保护、诊断和日志追溯基础上，修正 FairPropose /
FairUpdate 的缺席交易计票。详见 [论文复现审核](THEMIS_PAPER_AUDIT_2026-09-13.md)。
本次不改上报间隔、列表上限、邻接结构、worker 数或 hosting gate。
...
- 作者后续正文明确了缺席交易计票，当前已修正；旧版可能产生额外缺边或不同方向。
  使用旧版的部分接收实验、deferred microbenchmark 都需要重新评估。
  测试通过仍不能证明完整复现论文，尤其当前 hosting gate 不是 HotStuff。
...
6. 先明确主结论范围。若比较经过校准的性能，50 ms重复表现更好且有效时，统一用它
   重跑所需主曲线；不能逐点挑最好结果。若保留150 ms作为事先固定的实现配置，
   修正后的150 ms结果可以保留为该配置场景，同时报告50 ms复核；不得把150 ms结果
   描述为论文默认值、最优值或Themis的容量上限。若50 ms改变协议优劣结论，正文必须
   相应改写。旧版150 ms数据不能仅因配置相同就当作新版测量。
   若结果不一致，再补600 TPS，不扩展整套扫描。必要时延长运行确认没有持续积压。
